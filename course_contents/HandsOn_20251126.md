...
Next, we are going to update the downloader manager `apt-get` and installing the software that we need and can be found in apt-get (easier solution). In our case we need bowtie2 (the aligner) and samtools (sort and index bam files).
...
In total, four output files were generated:  
Running ```bowtie2``` generated the alignment file ```aligned_sorted.bam``` and the genome index ```aligned_sorted.bam.bai``` (the genome index has to be generated only once and can be re-used for additional analyses).
Running ```featureCounts```generated the ```gene_counts.txt``` which holds unnormalised raw read count data per gene along with additional information like gene length, strand etc. Out of 4305 genes, 2720 genes have a read count of at least 1; the remaining genes have zero counts.
The ```gene_counts.txt.summary```file details counting statistics. It shows that 39913 reads were assigned a genomic feature (gene) and counted. Reads that could not be assigned a genomic feature amounted to 3830 and 1840 reads were ambigious (aligned to more than one genomic feature).