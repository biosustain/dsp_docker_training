...
Running ```featureCounts```generated the ```gene_counts.txt``` which holds unnormalised raw read count data per gene along with additional information like gene length, strand etc. The ```gene_counts.txt.summary```file details counting statistics. It shows that 39913 reads were assigned a genomic feature (gene) and counted. Reads that could not be assigned a genomic feature amounted to 3830 and 1840 reads were ambigious (aligned to more than one genomic feature).